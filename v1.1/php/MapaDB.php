<?php
    ...
    class MapaDB extends PDO
    {
        ...
        public function getMapa($pkl, $dtFrom, $dtTo){
            $q = 'select id_reg, nome_estoque, num_picklist, data_hora from mapa, estoque where mapa.id_estoque = estoque.id_estoque';
            ...
            // var_dump($q_res);
            if ( $q_res ){
                $res = $q_res->fetchAll(PDO::FETCH_NAMED);
                if( count($res)>0 ){                
                    foreach ($res as $item) {
        ...
        public function postarMapa($pkl, $dthr, $stk){
            ...
            $q = "insert into mapa (num_picklist, data_hora, id_estoque) values ('$pkl', convert(datetime,'$dthr',120), $stk)";
            parent::query( $q );
