        ...
        if (reg.picklist.length > 0) {
            ...
            $http({
                ...
                function(response){
                    reg.ok = true;
                    reg.limparCampos();
                    console.log(reg.erro, "ResponseOK: ", response);
                ...
                function (response){
                    reg.erro = true;
                    reg.limparCampos();
                    console.log(reg.erro, "ResponseERRO: ", response);
        ...
        if (url != "../php/get-mapa.php?"){
            ...
            .then(function(resposta){
                if (resposta.data.length == 0) {
                    // console.log(resposta); //test line        
                    ctrl.lista = [];
                    ctrl.erroBusca = true;
                    $timeout(ctrl.limparCampos, 5000); //limpar campos em 5 seg
                }else{
                    console.log(resposta.data); //test line        
                    ctrl.erroBusca = false;
